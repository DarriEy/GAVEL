...
def read_data(read_path, Config_Path):
    ...
    df = fill_obs(df) #Fill in dataframe with potential missing observations
...
def QC0Snow(df, HS_path, Config_path):
    ...
    if HS_path != '':
        df_HS = fill_obs(pd.read_csv(HS_path, header =  int(SnowDict['headerrow']), skiprows = range(int(SnowDict['headerrow'])+1, int(SnowDict['startdata'])), index_col = 0, parse_dates = True))
        df['HS_q'] = df_HS['HS_q'].loc[df.index.values] #extract QC0 info for the index values
...
#Missing Data Test
def QC1_Miss(df, Param, ParamDict, QC = 'QC1'):
...
#Range Tests
def QC1_Range(df, Param, ParamDict, Config_Path, QC = 'QC1'):
...
def QC1_Stat_Range(df, Param, ParamDict, Config_Path, QC = 'QC1'):
    # Read the config for Parameter specific info and metadata
    Config_dict = read_config(Param, Config_Path)
...
#Step Tests
def QC1_Step(df, Param, ParamDict, Config_Path, QC = 'QC1'):
...
def QC1_Rep(df, Param, ParamDict, QC = 'QC1'):
    if Param != 'snow_depth':
        for para in ParamDict[Param]:
...
#Consistency Tests
def QC1_Cons(df, Param, ParamDict, Config_Path, QC = 'QC1'):
    # Read the config for Parameter specific info and metadata
    Config_dict = read_config(Param, Config_Path)
    Meta_dict = read_config('meta', Config_Path)
...
def QC1(df, Config_Path, QC = 'QC1'):
    ...
    for Param in Param_Range:
        ...
        if TestDict['missing'] == 'true':
            df = QC1_Miss(df, Param, ParamDict)

        if TestDict['range'] == 'true':
            df = QC1_Range(df, Param, ParamDict, Config_Path, QC = QC)

        if TestDict['stat_range'] == 'true':
            df = QC1_Stat_Range(df, Param, ParamDict, Config_Path, QC = QC)

        if TestDict['consistency'] == 'true':
            df = QC1_Cons(df, Param, ParamDict, Config_Path, QC = QC)

        if TestDict['repeating'] == 'true':
            df = QC1_Rep(df, Param, ParamDict, QC = QC)

        if TestDict['step'] == 'true':
            if Param in Param_Step:
                df = QC1_Step(df, Param, ParamDict, Config_Path, QC = QC)
